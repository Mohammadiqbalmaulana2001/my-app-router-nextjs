  ...
  return (
    <div className='grid grid-cols-4 mt-2 place items-center gap-5'>
        ...
        { product.data.map(item => (
            <Link href={`/product/detail/${item.id}`} key={item.id} className=" m-5 w-full max-w-sm my-5 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                
                    <img className="p-8 rounded-t-lg object-cover h-96 w-full" src={item.image} alt="product image" />
                
                <div className="px-5 pb-5">
                    
                        <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white truncate">{item.name}</h5>
